...
public class GetPlayerByName extends AsyncTask<String,Void,String> {
    ...
    public GetPlayerByName(TextView resultView, TextView debugView, ListView general, ImageView head, ProgressDialog prog, ProgressBar header, Context context, boolean uuidState){
        debug = debugView;
        result = resultView;
        mContext = context;
        details = general;
    ...
    private void parseGeneral(PlayerReply reply){
        ...
        if (MinecraftColorCodes.checkDisplayName(reply))
            ...
            resultArray.add(new ResultDescription("Name: ",reply.getPlayer().get("playername").getAsString()));
    ...
    private void parseParkourCounts(PlayerReply reply){
        ...
        for (Map.Entry<String, JsonElement> entry : parkourMain.entrySet()){
            ...
            if (parkArray.size() > 0){
                ...
                resultArray.add(new ResultDescription(entry.getKey().substring(0,1).toUpperCase() + entry.getKey().substring(1).toLowerCase(), "Click here to view parkour statistics", true, false, msg.toString()));
            }
        }
    }
    ...
    private void parseQuests(PlayerReply reply){
        ...
        for (Map.Entry<String, JsonElement> entry : questMain.entrySet()){
            ...
            if (qArray.size() > 0){
                ...
                resultArray.add(new ResultDescription(entry.getKey().substring(0,1).toUpperCase() + entry.getKey().substring(1).toLowerCase(), "Click here to see " + entry.getKey().substring(0,1).toUpperCase() + entry.getKey().substring(1).toLowerCase() + " quest statistics", true, false, msg.toString()));
            }
        }
    }
